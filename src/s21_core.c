...
// turn on bit in bits
int s21_bit_on(int bits, int bit)
{
    return bits | _2(bit % BITS_IN_INT);
}

// turn of bit int bits
int s21_bit_off(int bits, int bit)
{
    return bits & ~(_2(bit % BITS_IN_INT));
}
...
// get decimal scale
int s21_get_scale(s21 decimal decima)
{
    int exp = 0;
...
// set decimal scale
int s21_set_scale(s21_decimal *decim, int new_exp)
{
...
int s21_bit_swift_left(s21_decimal decim, int number, s21_decimal *result)
{
    ...
    {
        if (i != 0)
            buffer_bits = s21_swift_bits_right(decim.bits[i - 1],
...
int s21_truncate_to_scale(s21_decimal decim, int new_exp,
                          s21_decimal *result_decimal)
{
    int exp = s21_get_scale(decim);
    if (exp > 0)
        s21_set_scale(&decim, 0);
    int sign = s21_check_sign(decim);
    if (sign == 1)
        s21_chang_sign(&decim);
    int error_mark = 0;
