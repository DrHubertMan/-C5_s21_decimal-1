...
// + without scale, sign just 2 decimal
            ...
            s21_decimal *result_decimal)
{
    ...
    int scale = s21_get_scale(decim1);
    ...
    {
        s21_set_scale(result_decimal, scale);
    }
    return error_mark;
}
// sub but not scale or sign
            ...
            s21_decimal *result_decimal)
{
    ...
    s21_nullify_all_decimal(result_decimal);
    s21_balancing(&decim1, &decim2);
    int exp = s21_get_scale(decim1);
    error_mark = s21_sub_sign(decim1, decim2, result_decimal);
    s21_set_scale(result_decimal, exp);
    return error_mark;
}
                 ...
                 s21_decimal *result_decimal)
{
    ...
    while (!s21_is_null_decimal_bin(decim2) && !status)
    {
        ...
        if (!status)
            status = s21_bit_swift_left(decim1, 1, &decim1);
    }
            ...
            s21_decimal *result_decimal)
{
    ...
    else if (s21_get_scale(decim1) == 0 && s21_get_scale(decim2) == 0)
    {
        error_mark = s21_mul_lite(decim1, decim2, result_decimal);
    }
    else
    {
        int result_exp = s21_get_scale(decim1) + s21_get_scale(decim2);
        error_mark = s21_mul_lite(decim1, decim2, result_decimal);
        if (result_exp <= MAX_EXP)
            s21_set_scale(result_decimal, result_exp);
        else
            s21_nullify_all_decimal(result_decimal);
    }
    return error_mark;
...
int s21_int_div(s21_decimal decim1, s21_decimal decim2, s21_decimal *result)
{
    ...
    else
    {
        ...
        int res_exp = s21_get_scale(decim1) - s21_get_scale(decim2);
        s21_set_scale(&decim1, 0);
        s21_set_scale(&decim2, 0);
...
// full mod can do all functional
...
{
    s21_set_scale(&decim2, s21_get_scale(decim2) - status);
    int decim1_sign = s21_check_sign(decim1);
    int decim2_sign = s21_check_sign(decim2);
    ...
    while (s21_is_less_lite(mod_res, decim2))
        s21_bank_round(&decim2, 1);
...
int s21_div(s21_decimal decim1, s21_decimal decim2, s21_decimal *result)
{
        ...
        if (!s21_is_null_decimal_bin(mod_res))
        {
            ...
            for (int sub_stat = 0; !sub_stat; s21_nullify_all_decimal(&tmp))
            {
                ...
                if (!sub_stat)
                {
                    mod_res = tmp;
                    if (++exp < 29)
                        s21_set_scale(&mod_res, exp);
                }
            }
            if (!s21_get_scale(mod_res))
                s21_set_scale(&mod_res, 28);
            while (s21_is_less_lite(mod_res, decim2))
                s21_bank_round(&decim2, 1);
            s21_int_div(mod_res, decim2, &div_res);
            if (exp > 28)
                s21_set_scale(&div_res, exp - s21_get_scale(decim2));
            status = s21_add(div_res, *result, result);
        }
    }
    else
    ...
    if (status == 1 && s21_check_sign(decim1) ^ s21_check_sign(decim2))
        ++status;
    return status;
}